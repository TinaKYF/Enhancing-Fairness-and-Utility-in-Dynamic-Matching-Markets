...
class GeneticAlgorithm11:
    ...
    def __init__(self, startTime=0, endTime=0, updateTime=1, carParkMap=None, stayProList=None, clusterWaitList=None, clusterType=None,fairWeight=1):
    ...
    def execute(self):
        ...
        fileName = 'genetic11_' + self.clusterType+ '_fair_'+str(self.fairWeight) + '_result.csv'
        titles = ['runTime', 'stopTime', 'startTime', 'endTime', 'updateTime', 'popuSize', 'crossRate', 'mutateRate',
                  ...
                  'officerdis_record', 'stdofficerdis_record',"officerBenefit","stdofficerBenefit","clusterAverList","stdwait","waitList","stdWait"]
        params = [runTime, stopTime, self.startTime, self.endTime, self.updateTime, self.populationSize, self.crossRate,
                  ...
                  self.validCapturedTime, officerdis_record, stdofficerdis_record,officerBenefit, stdofficerBenefit,clusterAverList,stdClusterWait
                  , waitList, stdWait
                  ]
    ...
    def initialPopulations(self):
        ...
        '''
        统计各个cluster当前时间未被安排的request的数量
        '''
    ...
    def evolvePopulation(self):
        if self.freeOfficersNum == 1:
            self.localWaiting(self.populations[0][0])  # local optimisation
        elif (self.currentTime - self.startTime) % self.fairWeight == 0:
            for t in range(self.maxGen):
                ...
                populationscopy1 = copy.deepcopy(self.populations)
                populationscopy2 = copy.deepcopy(self.populations)
                self.computerStdWait(populationscopy2)
                populationscopy1.sort(key=lambda x: -x[1])
                populationscopy2.sort(key=lambda x: x[2])
                # for i in range(len(populationscopy2)):
                #     print(populationscopy2[i][2])
                    # Crossover
                for i in range(int(elitistNum), int(elitistNum + crossNum)):
                    newChild = self.generateChildChro(populationscopy1,populationscopy2)
                    if uniform(0, 1) > self.doLocRate:
                        self.localWaiting(newChild)
    ...
    def generateNewSolution(self, offisLeader):
        ...
        nodeClusterList = []
        for node in self.currentNodes:
            if node.center not in nodeClusterList:
                nodeClusterList.append(node.center)
        randomList = [i for i in range(len(nodeClusterList))]
        random.shuffle([i for i in range(len(nodeClusterList))])
        for index, center in zip(randomList, nodeClusterList):
            nodeClusterDict[center] = index
        ...
        agents = []
        for node in self.currentNodes:
            nodeDict[node.marker] = random.uniform(nodeClusterDict[node.center], nodeClusterDict[node.center] + 1)
        for driver in self.freeOfficers:
            nodeDict[driver.id_] = random.uniform(0, len(nodeClusterList))
        for cell in self.cells_id:
            agents.append(nodeDict[cell])
    ...
    def generateChildChro(self,population1,population2):
        crossNum = self.populationSize * self.crossRate
        child1Chro = []
        a = -1
        b = -1
        ...
        parent1Chro = copy.deepcopy(population1[a][0])
        parent2Chro = copy.deepcopy(population2[b][0])
    ...
    def computerStdWait(self, population):
        officerdis = []


        '''
        先假设所有的request都不会被满足，然后通过减去被安排的request来计算waiting_time
        '''
        for popuindex in range(len(population)):
            clusterWaitListCopy = copy.deepcopy(self.clusterWaitList)
            clusterNumListCopy = copy.deepcopy(self.clusterNumList)
            # for i in range(len(clusterNumListCopy)):
            #     clusterWaitListCopy[i] += clusterNumListCopy[i]
            clusterAverList=[]
            ...
            for eleindex in range(len(path) - 1):
                ...
                if 'O' in path[eleindex][0]:
                    ...
                    if 'O' not in path[eleindex + 1][0]:
                        ...
                        node = self.carParkMap.findNodeByMarkerId(nodeid)
                        clusterWaitListCopy[node.center]-=1
            for i in range(len(clusterWaitListCopy)):
                if clusterNumListCopy[i]+self.clusterAssignList[i]==0:
                    clusterAverList.append(clusterWaitListCopy[i]/1)
                else:
                    clusterAverList.append(clusterWaitListCopy[i]/(clusterNumListCopy[i]+self.clusterAssignList[i]))
            stdWait = statistics.stdev(clusterAverList)
            solution.append(stdWait)

    def localWaiting(self, path):
        """
        Reorder points in each sub path with max pro from the previous node
        :param path: a path reference
        """
            ...
            if 'O' in ele[0] or i == pathEndNodeI:
                ...
                if nodesNum > 1:
                    ...
                    for j in range(1, nodesNum):  # compare n - 1 times
                        ...
                        winner = None
                        nStartI = offiIndex + j
                        leadNode=self.getCurrentNodeById(path[nStartI][0])
                        clusterId=leadNode.center
                        clusterDict, clusterList = self.computeClusterWaitng(leadNode)
                        if len(clusterList)<2:
                            continue
                        for k in range(nStartI, subPathEndI + 1):  # indexes on the sub path
                            pId = path[k][0]
                            nextNode = self.getCurrentNodeById(pId)
                            if nextNode.center!=clusterId:
                                continue
                            # pro, recordId = nextNode.calculateProbability(arriveTime, self.currentTime,
                            #                                               self.stayProList)
                            clusterListCopy=copy.deepcopy(clusterList)
                            clusterListCopy[clusterDict[nextNode.marker]]-=1
                            stdWaiting=statistics.stdev(clusterListCopy)

                            if stdWaiting < proMax:
                                proMax = stdWaiting
                                winner = [k, pId]
                        if winner:
                            winnerIndex = winner[0]
    ...
    def computeClusterWaitng(self,node):
        nodeMarker=node.marker
        clusterId=node.center
        clusterList=[]
        clusterDict={}
        count=0
        for nodes in self.carParkMap.nodes:
            if node.requestTime>self.currentTime:
                if nodes.center==clusterId:
                    if nodes.takeTime and nodes.takeTime<self.currentTime:
                        clusterList.append(nodes.takeTime-nodes.requestTime)
                    else:
                        clusterList.append(self.currentTime+1-nodes.requestTime)
                    clusterDict[nodes.marker]=count
                    count+=1
        return (clusterDict,clusterList)
